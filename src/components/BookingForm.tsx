...
export function BookingForm() {
  ...
  const geolocationTimeoutRef = useRef<NodeJS.Timeout>();
  ...
  const initializeMap = useCallback(async () => {
    ...
    try {
      ...
      setErrors(prev => ({
        ...prev,
        maps: 'Failed to load Google Maps. Please check your internet connection and try again.'
      }));
    } finally {
  ...
  const getCurrentLocation = useCallback(async () => {
    ...
    try {
      ...
      if (response.results[0]) {
        setFormData(prev => ({ ...prev, pickup: response.results[0].formatted_address }));
      ...
      setErrors(prev => ({
        ...prev,
        geocoding: `Erreur de géolocalisation : ${error instanceof Error ? error.message : 'Unknown error'}`
      }));
    } finally {
      setIsGettingLocation(false);
      if (geolocationTimeoutRef.current) {
  ...
  const calculateRoute = useCallback(() => {
    ...
    if (formData.pickup && formData.dropoff) {
      ...
      // Verify if addresses have geometry
      ...
      directionsServiceRef.current.route(request, (result, status) => {
        if (status === google.maps.DirectionsStatus.OK && result) {
          ...
          const price = formData.bookingType === 'distance'
            ? 20 + (distanceInKm * 2.5)
            : formData.hours * 70;
          ...
          setErrors(prev => ({
            ...prev,
            geocoding: "Impossible de calculer l'itinéraire. Veuillez vérifier les adresses."
          }));
        }
      });
    }
    ...
    if (formData.price) {
      ...
      try {
        const bookingData: Booking = {
          ...
          hours: formData.bookingType === 'hourly' ? formData.hours : undefined,
          distance: formData.bookingType === 'distance' ? formData.distance : undefined,
        ...
        if (booking.id) {
          await supabase
            .from('bookings')
            .update({ payment_intent_id: paymentData.clientSecret })
            .eq('id', booking.id);
        }
    ...
    try {
      await emailjs.send(
        ...
        'template_wrd97hj',
        {
          ...
          booking_type: formData.bookingType === 'distance' ? 'Distance-based' : 'Hourly',
          name: formData.name,
          phone: formData.phone,
          email: formData.email,
          pickup: formData.pickup,
          ...
          distance: formData.bookingType === 'distance' ? `${formData.distance?.toFixed(2)} km` : 'N/A',
          hours: formData.bookingType === 'hourly' ? formData.hours : 'N/A',
      ...
      setErrors(prev => ({
        ...prev,
        email: error instanceof Error ? error.message : 'An error occurred sending the email'
      }));
      return false;
    }
  }, [formData]);
  ...
  const formSections = useMemo(() => ({
    ...
    locationSection: (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label htmlFor="pickup" className="block font-semibold text-gray-700">
            {t('booking.pickup')}*
          </label>
          <div className="relative">
            ...
            >
              <MapPin className={`w-6 h-6 ${isGettingLocation ? 'animate-pulse' : ''}`} />
            </button>
          </div>
        </div>

        <div className="space-y-2">
          <label htmlFor="dropoff" className="block font-semibold text-gray-700">
            {t('booking.dropoff')}*
          </label>
          <input
            type="text"
  ...
  }), [formData.bookingType, formData.pickup, formData.dropoff, handleInputChange, getCurrentLocation, isGettingLocation, t]);
    ...
    <>
      <div className="max-w-4xl mx-auto mb-8">
        {errors.maps ? (
          ...
          <div className="w-full h-[400px] rounded-lg shadow-lg mb-8" ref={mapContainerRef}>
            {!mapsLoaded && (
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-6">
        ...
        {formData.bookingType === 'hourly' && (
          <div className="space-y-2">
            <label htmlFor="hours" className="block font-semibold text-gray-700">
              {t('booking.hours')}*
            </label>
            <input
              type="number"
        ...
        <div className="space-y-2">
          <label htmlFor="flightNumber" className="block font-semibold text-gray-700">
        ...
        <div className="space-y-2">
          <label htmlFor="notes" className="block font-semibold text-gray-700">
            {t('booking.notes')}
          </label>
          <textarea
            id="notes"
        ...
        >
          {isSubmitting ? t('booking.submitting') : paymentCompleted ? t('booking.paid') : t('booking.proceedToPayment')}
        </button>
      </form>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg max-w-md w-full mx-4">
            <h3 className="text-xl font-semibold mb-4">{t('booking.success')}</h3>
